...
namespace Antics.Patcher.Setup
{
	public class PatchTask : SetupOperation
	{
		...
		private DiffPatch.Patcher.Mode mode;
		...
		public override void Run()
		{
			...
			mode = (DiffPatch.Patcher.Mode) Settings.Default.PatchMode;
			...
			if (fuzzy > 0 || mode == DiffPatch.Patcher.Mode.FUZZY && failures > 0)
		...
		private FilePatcher Patch(string patchPath)
		{
			...
			foreach (var result in patcher.results) {
				...
				if (result.mode == DiffPatch.Patcher.Mode.FUZZY || result.offsetWarning) warnings++;
				if (result.mode == DiffPatch.Patcher.Mode.EXACT) exact++;
				else if (result.mode == DiffPatch.Patcher.Mode.OFFSET) offset++;
				else if (result.mode == DiffPatch.Patcher.Mode.FUZZY) fuzzy++;
			}
